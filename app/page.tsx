...
import { Search, User, ShoppingCart, MapPin, Phone, Clock, Shield, HelpCircle, Truck, Star, ChevronRight, ChevronLeft, Check, Mail, Instagram, Facebook, MessageCircle, Heart, Share2, Loader2, Eye, Sparkles, Zap, Bell, ArrowRight, ExternalLink, RefreshCw, Home, Headphones, Accessibility, Package, Copy, Paintbrush, Plus, Minus } from "lucide-react"
...
const Header = ({ page, setPage, cartCount, onCartOpen }) => (
  <header style={{ background: "#1a1464", padding: "10px 16px", position: "sticky", top: 0, zIndex: 100, boxShadow: "0 2px 8px rgba(0,0,0,0.3)" }}>
    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
      ...
      {/* Logo */}
      <div onClick={() => setPage("home")} style={{ cursor: "pointer", marginRight: "auto" }}>
      ...
      {/* Search box */}
      <div style={{ flex: 1, maxWidth: 180, background: "white", borderRadius: 4, display: "flex", alignItems: "center", padding: "5px 10px", gap: 6 }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 8 }}>
      {CATEGORIES.map(cat => (
        <div key={cat.name} onClick={() => onCategoryClick(cat.name)}
          ...
          <div style={{ width: 52, height: 52, borderRadius: 12, background: "#e8ecf5", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, transition: "background 0.2s" }}>
            {cat.icon}
          </div>
...
const FeaturedProducts = ({ onAdd, favorites, onToggleFavorite }) => (
  <div style={{ background: "#f7f8fc", padding: "20px 16px" }}>
    <h2 style={{ fontSize: 18, fontWeight: 700, textAlign: "center", marginBottom: 16, color: "#222" }}>Mais Vendidos</h2>
        ...
        <div key={p.id} style={{ background: "white", borderRadius: 10, border: "1px solid #e5e7eb", overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
          ...
          <div style={{ padding: "10px 10px 12px" }}>
            ...
            <button onClick={() => onAdd(p)}
              style={{ width: "100%", background: "#1a1464", color: "white", border: "none", borderRadius: 6, padding: "8px 0", fontSize: 12, fontWeight: 700, cursor: "pointer", letterSpacing: 0.5 }}>
              COMPRAR
            </button>
          </div>
        </div>
...
const CategorySection = ({ onCategoryClick }) => (
  <div style={{ background: "white", padding: "20px 16px" }}>
    <h2 style={{ fontSize: 18, fontWeight: 700, textAlign: "center", marginBottom: 16, color: "#222" }}>Categoria de Tintas</h2>
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
      {["paredes internas", "paredes internas", "paredes internas", "paredes internas", "paredes internas", "paredes internas"].map((label, i) => (
        <button key={i} onClick={() => onCategoryClick("Tintas")}
          style={{ border: i === 0 ? "2px solid #1a1464" : "1px solid #d1d5db", borderRadius: 20, padding: "8px 10px", fontSize: 11, color: i === 0 ? "#1a1464" : "#555", background: "white", cursor: "pointer", fontWeight: i === 0 ? 700 : 400 }}>
          {label}
        </button>
      ))}
    </div>
...
const ProductsPage = ({ onAdd, favorites, onToggleFavorite }) => {
...
export default function App() {
  ...
  const goToCategory = (cat) => { setPage("produtos") }
